...
package org.freedesktop.gstreamer.swt.overlay;
...
import org.freedesktop.gstreamer.Bin.ELEMENT_ADDED;
...
public class VideoComponent extends Canvas implements BusSyncHandler, DisposeListener {
     ...
     * @param parent
     * @param style
     * @param enableX11Events
     *            true if X11 event should have to be grabbed (mouse move, enter
     *            and leave event on Linux).
     * 
     *            On Linux by default the handling of mouse move, enter and
     *            leave event are not propagated. Unfortunately the
     *            "handle-events" properties hide some important expose events
     *            too, sowe've to do some lowlevel trick to be able to get these
     *            events.
     */
    public VideoComponent(final Composite parent, int style, boolean enableX11Events) {
        super(parent, style | SWT.EMBEDDED);
        this.setBackground(org.eclipse.swt.widgets.Display.getDefault().getSystemColor(SWT.COLOR_BLACK));
        x11Events = enableX11Events;
        addDisposeListener(this);

        autosink = (Bin) ElementFactory.make("autovideosink", "VideoComponent" + counter++);
        sink.add(autosink);
        sink.addPad(new GhostPad("sink", autosink.getSinkPads().get(0)));
        ...
        sinkListener = new ELEMENT_ADDED() {
            public void elementAdded(Bin bin, Element element) {
                if (element instanceof BaseSink) {
                    ...
                    // bus.setSyncHandler(VideoComponent.this); // for
                    // prepare-xwindow-id
                    setOverlay();
    ...
    public void widgetDisposed(DisposeEvent arg0) {
        ...
        if (videosink != null && !videosink.getState().equals(State.NULL))
            throw new IllegalStateException(
                    "Wrong state on dispose: " + videosink.getState() + " -> " + videosink.getName());
    }
    ...
    public BusSyncReply syncMessage(Message message) {
        if (!SWTOverlay.isVideoOverlayPrepareWindowHandleMessage(message)) {
    ...
    /**
     * Tell an overlay that it has been exposed. This will redraw the current
     * frame in the drawable even if the pipeline is PAUSED.
     */
    public void expose() {
    ...
    /**
     * On Linux by default the handling of mouse move, enter and leave event are
     * not propagated. Unfortunately the "handle-events" properties hide some
     * important expose events too, sowe've to do some lowlevel trick to be able
     * to get these events. In this case we (gstreamer-linux) must handle redraw
     * too!
     *
     * @param enableX11Events
     *            true if X11 event should have to be grabbed (mouse move, enter
                ...
                public void run() {
                    try {
                        ...
                        x11.XSelectInput(display, window,
                                new NativeLong(X11.ExposureMask | X11.VisibilityChangeMask | X11.StructureNotifyMask
                                        | X11.FocusChangeMask |
                                ...
                                getDisplay().asyncExec(new Runnable() {
                                    public void run() {
                                        if (watcherRunning && !isDisposed()) {
                                            ...
                                            // XMotionEvent e =
                                            // (XMotionEvent)xEvent.readField("xmotion");
                                            // swtEvent.x = e.x;
                                            // swtEvent.y = e.y;
                                            // notifyListeners(SWT.MouseMove,
                                            // swtEvent);
                                            // break;
